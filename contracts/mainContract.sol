...
contract TradeRegulation is Ownable{
    ...
    function uploadInfo (uint pI, address next, string location, bytes photo)
    {
  ...
  */
  struct Tx{
   ...
   function upload(bytes32 uid, address sender, bytes32 docType, bytes _hash) returns (bool){
